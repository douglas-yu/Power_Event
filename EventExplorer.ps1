...
function Show-FilterDialog {
    ...
    function MkChk($t,$x,$fg,$lvl) {
        ...
        $c.Text = $t; $c.Location = New-Object Drawing.Point($x,26)
        $c.AutoSize = $true; $c.ForeColor = $fg
        $c.Checked = $global:FilterCfg.Levels -contains $lvl
        return $c
    }
    ...
    foreach ($c in @($dtFrom,$dtTo)) { $c.BackColor = $C.Bg; $c.ForeColor = $C.Text }
    ...
    $btnReset.Add_Click({
        foreach ($c in @($chkCrit,$chkErr,$chkWarn,$chkInfo,$chkVerb)) { $c.Checked = $true }
