...
$splitMain.SplitterDistance = [int]($form.ClientSize.Width * 0.20)
$splitMain.SplitterWidth    = 5
# SplitterDistance is set to 20% in Add_Shown after the form renders
...
# Keep left panel at 20% on window resize
$form.Add_Resize({
...
# Set left panel to exactly 20% of client width once form is fully rendered
$form.Add_Shown({
    $newDist = [int]($form.ClientSize.Width * 0.20)
    $minOk   = $newDist -ge $splitMain.Panel1MinSize
    $maxOk   = $newDist -le ($splitMain.Width - $splitMain.Panel2MinSize - $splitMain.SplitterWidth)
