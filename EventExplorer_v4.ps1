...
# ==============================================================================
#  CORE: LOAD EVENTS  (async - runs Get-WinEvent in background runspace)
# ==============================================================================
function Load-Events {
    ...
    if ($global:LoadTimer -and $global:LoadTimer.Enabled) {
        $global:LoadTimer.Stop()
        $global:LoadTimer.Dispose()
        $global:LoadTimer = $null
        try { if ($global:LoadPS) { $global:LoadPS.Stop()    } } catch {}
        try { if ($global:LoadPS) { $global:LoadPS.Dispose() } } catch {}
        $global:LoadPS = $null
    }

    # Show marquee progress bar
    $pbLoad.Style                = [Windows.Forms.ProgressBarStyle]::Marquee
    $pbLoad.MarqueeAnimationSpeed = 25
    $pbLoad.Value                = 0
    $pbLoad.Visible              = $true
    $statusLabel.Text            = "[...] Loading $ComputerName -> $LogName ..."
    $form.Refresh()

    # Build plain hashtable filter (OrderedDictionary not accepted by Get-WinEvent)
    ...
    # Synchronized hashtable passes results back from the background runspace
    ...
    # Script executed in background runspace (no access to outer scope variables)
    $bgScript = {
        param($syncHash, $filter, $safeMax, $ComputerName, $isLocal, $Credential)
        try {
            $params = @{
            ...
            $syncHash.Events = @(Get-WinEvent @params)
        } catch {
            $syncHash.ErrorMsg = $_.Exception.Message
        } finally {
            $syncHash.Done = $true
        }
    }
    ...
    # Polling timer - checks every 150 ms whether the runspace finished
    $pollTimer = New-Object Windows.Forms.Timer
    $pollTimer.Interval = 150

    $pollTimer.Add_Tick({
        $ctx  = $global:LoadContext
        $sync = $ctx.SyncHash
        if (-not $sync.Done) { return }          # still running, keep waiting

        $global:LoadTimer.Stop()

        # Switch progress bar to determinate completion
        ...
        if ($sync.ErrorMsg) {
            ...
            if ($msg -match "No events were found") {
                ...
                [Windows.Forms.MessageBox]::Show(
                    "Failed to load events from '$($ctx.ComputerName) / $($ctx.LogName)':`n`n$msg",
                    "Load Error","OK","Error")
            }
        } else {
            $global:Events     = if ($sync.Events) { $sync.Events } else { @() }
            $global:CurrentLog = @{
            ...
            $statusLabel.Text   = "[OK] Loaded $($global:Events.Count) events -- $($ctx.ComputerName) / $($ctx.LogName)"
            $evtCountLabel.Text = "Events: $($global:Events.Count)"
        }

        # Auto-hide progress bar after 900 ms so user sees the green fill
        ...
        try { $global:LoadPS.EndInvoke($global:LoadHandle) } catch {}
...
# Progress bar (hidden when idle, marquee while loading, fills green on done)
$pbLoad                       = New-Object Windows.Forms.ToolStripProgressBar
$pbLoad.Size                  = New-Object Drawing.Size(180, 14)
$pbLoad.Style                 = [Windows.Forms.ProgressBarStyle]::Marquee
$pbLoad.MarqueeAnimationSpeed = 25
$pbLoad.Minimum               = 0
$pbLoad.Maximum               = 100
